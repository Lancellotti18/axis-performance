...
async def get_full_report_data(project_id: str, user: dict = Depends(require_user)):
    """
    ...
    # Project (owner only — the report leaks address, costs, client details)
...
async def get_daily_logs(project_id: str, user: dict = Depends(require_user)):
...
async def save_report_overrides(project_id: str, payload: OverridesPayload, user: dict = Depends(require_user)):
...
async def export_report_pdf(project_id: str, user: dict = Depends(require_user)):
    """Generate a clean PDF of the full project report and stream it back."""
    from fastapi import Request

    # Fetch the full data (ownership enforced inside)
    data = await get_full_report_data(project_id, user)
    ...
    try:
        ...
        elems.append(Paragraph(
            "Generated by Axis Roofing Performance · All cost estimates and compliance items should be verified with licensed professionals before construction.",
            caption
        ))

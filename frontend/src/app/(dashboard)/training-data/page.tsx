...
const QUALITY_COLORS: Record<string, string> = {
  unverified: 'bg-[#e4e4e2] text-[#1a1a1a]',
...
export default function TrainingDataPage() {
  ...
  return (
    <div className="mx-auto max-w-[1400px] space-y-6 p-6 text-[#1a1a1a]">
      ...
      {stats && (
        <section>
          <h2 className="mb-2 text-sm font-semibold text-[#1a1a1a]">Per task type</h2>
          <ul className="grid grid-cols-1 gap-2 md:grid-cols-2 lg:grid-cols-3">
            {Object.entries(TASK_LABELS).map(([key, label]) => {
              ...
              return (
                <li
                  key={key}
                  className="rounded-lg border border-[#dededc] bg-[#f8f8f7] p-3"
                >
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <div className="text-xs uppercase tracking-wide text-[#6b7280]">{key}</div>
                      <div className="text-sm font-medium text-[#1a1a1a]">{label}</div>
                    </div>
                    <div className="shrink-0 text-right">
                      <div className="text-xl font-bold text-[#1a1a1a]">{count}</div>
                      <div className="text-[10px] text-[#6b7280]">examples</div>
                    </div>
                  </div>
                  <button
      ...
      <section>
        <div className="mb-3 flex items-center justify-between gap-2">
          <h2 className="text-sm font-semibold text-[#1a1a1a]">Recent examples</h2>
        ...
        {examples.length === 0 ? (
          <p className="rounded border border-[#dededc] bg-[#f8f8f7] p-4 text-xs text-[#6b7280]">
          ...
          <ul className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
            {examples.map(ex => (
              <li
                key={ex.id}
                className="overflow-hidden rounded-lg border border-[#dededc] bg-[#f8f8f7]"
              >
                ...
                <div className="space-y-2 p-3 text-xs">
                  <div className="flex items-center justify-between">
                    <span className={`rounded px-2 py-0.5 text-[10px] ${QUALITY_COLORS[ex.quality_tier] || 'bg-[#e4e4e2]'}`}>
                      {ex.quality_tier}
                    </span>
                  ...
                  <div>
                    <div className="font-medium text-[#1a1a1a]">{TASK_LABELS[ex.task_type] || ex.task_type}</div>
                    <div className="text-[#6b7280] capitalize">{ex.capture_source.replace('_', ' ')}</div>
                  </div>
      ...
      <div className="rounded-lg border border-blue-400/30 bg-blue-500/5 p-4 text-xs text-[#2d2d2d]">
...
function StatCard({ label, value, accent = false, color }: { label: string; value: number; accent?: boolean; color?: string }) {
  return (
    <div className={`rounded-lg border ${accent ? 'border-blue-400/40 bg-blue-500/10' : 'border-[#dededc] bg-[#f8f8f7]'} p-3`}>
      <div className="text-[10px] uppercase tracking-wide text-[#6b7280]">{label}</div>
      <div className={`text-2xl font-bold ${color || 'text-[#1a1a1a]'}`}>{value.toLocaleString()}</div>
    </div>
  )
}

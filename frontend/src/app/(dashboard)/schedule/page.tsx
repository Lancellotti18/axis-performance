    ...
    <div className="min-h-full p-6">
      <div className="mb-5 flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-xs text-[#6b7280]">Free inspections your homeowners booked from their roof reports.</p>
        </div>
      </div>

      {appts === null ? (
        <div className="flex h-64 items-center justify-center text-[#6b7280]">Loading…</div>
      ) : (
        <div className="grid gap-5 xl:grid-cols-[1.7fr_1fr]">
          {/* Calendar */}
          <div className="rounded-2xl border border-[#dededc] bg-[#f8f8f7] p-5"
            style={{ boxShadow: '0 8px 40px rgba(0,0,0,0.35)' }}>
            <div className="mb-4 flex items-center justify-between">
              <div className="text-lg font-bold text-[#1a1a1a]">{monthLabel}</div>
            ...
            <div className="grid grid-cols-7 gap-1.5">
              {DOW.map(d => <div key={d} className="pb-1.5 text-center text-[11px] font-semibold uppercase tracking-wider text-[#6b7280]">{d}</div>)}
              {cells.map(d => {
                const key = toKey(d)
                ...
                return (
                  <button key={key} onClick={() => setSelected(key)}
                    className={`flex h-[120px] flex-col overflow-hidden rounded-xl border p-2 text-left transition ${ isSel ? 'border-blue-400/70 bg-blue-500/15 ring-1 ring-blue-400/40' : 'border-[#dededc] hover:border-[#dededc] hover:bg-[#f8f8f7]' } ${inMonth ? '' : 'opacity-35'}`}>
                    <span className={`text-sm font-bold ${isToday ? 'flex h-6 w-6 items-center justify-center rounded-full bg-blue-600 text-white' : 'text-[#2d2d2d]'}`}>{d.getDate()}</span>
                    ...
                    <div className="mt-1 flex min-h-0 flex-1 flex-col gap-0.5">
                      ...
                      {active.length > 3 && (
                        <span className="pl-1 text-[10px] font-semibold text-[#6b7280]">+{active.length - 3} more</span>
                      )}
                    </div>
                  </button>
                )
          ...
          <div className="space-y-5">
                ...
                <div className="space-y-1.5">
                  {upcoming.map(a => (
                    <button key={a.id} onClick={() => { setSelected(a.preferred_date); setCursor(new Date(new Date(a.preferred_date + 'T00:00:00').getFullYear(), new Date(a.preferred_date + 'T00:00:00').getMonth(), 1)) }}
                      className="flex w-full items-center gap-2 rounded-lg bg-[#f8f8f7] px-2.5 py-2 text-left ring-1 ring-[#dededc] hover:bg-[#f8f8f7]">
            ...
            <div className="rounded-2xl border border-[#dededc] bg-[#f8f8f7] p-4">
              ...
              {dayAppts.length === 0 ? (
                <p className="py-8 text-center text-xs text-[#6b7280]">No inspections booked this day.</p>
              ) : (
                <div className="space-y-3">
                  {dayAppts.map(a => {
                    ...
                    const L = a.lead
                    return (
                      <div key={a.id} className="rounded-xl bg-[#f8f8f7] p-3.5 ring-1 ring-[#dededc]">
                        <div className="flex items-start justify-between gap-2">
                          <div className="min-w-0">
                            ...
                            <div className="text-[11px] text-[#6b7280]">🕑 {WINDOW_LABEL[a.time_window] || a.time_window}{a.address && <> · {a.address}</>}</div>
                          </div>
                        ...
                        {L && (L.work_type || L.condition || L.roof_age || (L.issues?.length ?? 0) > 0 || L.squares || L.price_low) && (
                          <div className="mt-2.5 rounded-lg bg-black/20 p-2.5">
                            ...
                            <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-[#6b7280]">
                              {L.squares != null && <span>📐 <strong className="text-[#1a1a1a]">{L.squares}</strong> squares{L.roof_sqft ? ` · ${Math.round(L.roof_sqft).toLocaleString()} ft²` : ''}</span>}
                              {L.price_low != null && L.price_high != null && <span>💵 Quoted <strong className="text-[#1a1a1a]">{money(L.price_low)}–{money(L.price_high)}</strong></span>}
                            </div>
                          </div>
                        )}
                        ...
                        <div className="mt-2.5 flex flex-wrap items-center gap-1.5">
                          ...
                          {(a.status === 'requested' || a.status === 'confirmed') && (
                            <button onClick={() => { setProposeFor(proposeFor === a.id ? null : a.id); setProposeDates(['', '', '']) }}
                              className="rounded-lg bg-[#eeeeed] px-2.5 py-1 text-[11px] font-medium text-[#2d2d2d] ring-1 ring-[#dededc] hover:bg-[#eeeeed]">
                              Can’t make it?
                            </button>
                          )}
                        </div>

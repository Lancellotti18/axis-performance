              ...
              {cells.map(d => {
                ...
                return (
                  <button key={key} onClick={() => setSelected(key)}
                    className={`flex min-h-[92px] flex-col rounded-xl border p-2 text-left transition ${
                    ...
                    <div className="mt-1 flex flex-1 flex-col justify-end gap-0.5">
                      {active.slice(0, 2).map(a => (
                        <span key={a.id} className={`flex items-center gap-1 truncate rounded px-1 py-0.5 text-[10px] font-medium ${STATUS_STYLE[a.status].chip}`}>
                          <span className={`h-1.5 w-1.5 shrink-0 rounded-full ${STATUS_STYLE[a.status].dot}`} />
                          <span className="truncate">{(a.homeowner_name || 'Homeowner').split(' ')[0]}</span>
                        </span>
                      ))}

    ...
    >
      <div className="mx-auto max-w-3xl">
        ...
        <section className="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_10px_40px_-12px_rgba(15,40,80,0.12)] sm:p-6">
          ...
          <div className="mt-1 text-sm text-[#6b7280]">{current.blurb}</div>
            ...
            {p.stages.map((s, i) => {
              ...
              return (
                <li key={s} className="relative flex items-start gap-3 pb-4 last:pb-0">
                  ...
                  <span className={`relative z-10 flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[11px] font-bold ${ done ? 'bg-emerald-500 text-white' : active ? 'bg-blue-600 text-white ring-4 ring-blue-100' : 'bg-slate-200 text-[#6b7280]' }`}>{done ? '✓' : i + 1}</span>
                  <span className={`pt-0.5 text-sm ${active ? 'font-semibold text-slate-900' : done ? 'text-[#9ca3af]' : 'text-[#6b7280]'}`}>
                    {info.label}
                  </span>
                </li>
              )
          ...
          <section className="mb-6 grid grid-cols-3 gap-3">
            {[
              ...
              <div key={x.l} className="rounded-xl border border-slate-200 bg-white p-3 text-center shadow-sm">
                ...
                <div className="mt-0.5 text-[10px] uppercase tracking-wide text-[#6b7280]">{x.l}</div>
              </div>
            ))}
          </section>
        )}
        ...
        {p.report_url && (
          <section className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div>
              ...
              <div className="mt-0.5 text-xs text-[#6b7280]">Full measurement report — keep it for your records and insurance.</div>
            </div>
        ...
        <section className="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div ref={threadRef} className="max-h-72 space-y-2 overflow-y-auto rounded-lg bg-slate-50 p-3 ring-1 ring-slate-100">
            {messages.length === 0 && (
              <p className="py-4 text-center text-xs text-[#6b7280]">No messages yet — say hello 👋</p>
            )}
            {messages.map(m => (
              <div key={m.id} className={`flex ${m.sender === 'homeowner' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] rounded-2xl px-3.5 py-2 text-sm shadow-sm ${ m.sender === 'homeowner' ? 'rounded-br-md bg-blue-600 text-white' : 'rounded-bl-md bg-white text-slate-800 ring-1 ring-slate-200' }`}>
                  <div className="whitespace-pre-wrap break-words">{m.body}</div>
                  <div className={`mt-1 text-[10px] ${m.sender === 'homeowner' ? 'text-blue-200' : 'text-[#6b7280]'}`}>
        ...
        <p className="mt-8 text-center text-[10px] text-[#6b7280]">Powered by Axis Roofing Performance.</p>
      </div>
    </main>
  )
}

...
export default function CrewGalleryPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (error) return (
    <div className="flex min-h-screen items-center justify-center bg-[#040810] p-8 text-center">
      <div>
        <div className="text-lg font-semibold text-white">Photos unavailable</div>
        <div className="mt-1 text-sm text-slate-400">{error}</div>
      </div>
    </div>
  )
  if (!data) return (
    <div className="flex min-h-screen items-center justify-center bg-[#040810] text-sm text-slate-500">Loading photos…</div>
  )
    ...
    <div className="min-h-screen bg-[#040810]">
      <div className="mx-auto max-w-3xl p-4 sm:p-6">
        <header className="mb-5 border-b border-white/10 pb-4">
          ...
          <h1 className="mt-1 text-xl font-bold text-white">{project.name || 'Project'}</h1>
          {(project.address || project.city) && (
            <p className="text-sm text-slate-400">{[project.address, project.city].filter(Boolean).join(', ')}</p>
          )}
        </header>

        {photos.length === 0 && <div className="py-16 text-center text-sm text-slate-500">No photos posted yet.</div>}
              ...
              <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3">
                {inPhase.map(p => (
                  <button key={p.id} onClick={() => setZoom(p)}
                    className={`group relative aspect-square overflow-hidden rounded-lg bg-slate-900 ring-1 ${meta.ring}`}>
                    {p.url
      ...
      {zoom && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black/92 p-4" onClick={() => setZoom(null)}>
          ...
          {zoom.caption && <div className="mt-3 max-w-lg text-center text-sm text-slate-200">{zoom.caption}</div>}
          <button onClick={() => setZoom(null)} className="mt-3 rounded-lg bg-white/10 px-4 py-2 text-sm text-white hover:bg-white/20">Close</button>
        </div>
      )}
    </div>
  )
